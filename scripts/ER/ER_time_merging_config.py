...
plt.axhline(np.mean(beta_DS_estimated[3:]), 0, np.max(N_list)*1.1, color='k', linestyle='--', label=fr'$\beta_{{DS}}={np.mean(beta_DS_estimated[3:]):.4f}\pm {2*np.std(beta_DS_estimated[3:]):.4f}$ ($N\geq${N_list[3]})')
...
plt.savefig(save_path_fig + "ER_beta_damage_spreading.pdf", dpi=300)
plt.close()
...
fig.savefig(save_path_fig + f"ER_damage_spreading_coal.pdf", dpi=300)
plt.close()