...
max_beta = beta_true * 1.5

# Split betas to get points around beta_true
betas_lower = np.linspace(0, beta_true, 4)[:-1]
betas_upper = np.linspace(beta_true, max_beta, 10)
...
for N in [100, 200, 500, 750]:
    print(f"Processing ER model with N={N}...")
...
# Plotting the results for the Erdős-Rényi model
...
plt.vlines(beta_true, 100, 750, color='k', linestyle='--', label=r'$\beta=\tanh^{-1}(1/d)$')
plt.colorbar(label='Coalescence Time')
plt.xlabel(r'Inverse Temperature $\beta$')
plt.ylabel('System Size $N$')
plt.savefig(save_path + f'temp/ER_coal_time_heatmap_d_{d}.png')
plt.savefig(save_path + f'temp/ER_coal_time_heatmap_d_{d}.svg')
...
plt.vlines(beta_true, 0, max(coalescence_times_matrix.flatten())*10, color='k', linestyle='--', label=r'$\beta=\tanh^{-1}(1/d)$')
plt.ylim(min(coalescence_times_matrix.flatten())/2, max(coalescence_times_matrix.flatten())*2)
plt.xlabel(r'Inverse Temperature $\beta$')
plt.yscale('log')
plt.ylabel('Coalescence Time')
plt.legend()
plt.savefig(save_path + f'temp/ER_coal_time_vs_beta_d_{d}.png')
plt.savefig(save_path + f'temp/ER_coal_time_vs_beta_d_{d}.svg')
...
for i, beta in enumerate(betas):
    ...
    ax.plot(sorted(T_coal_vs_beta_N.keys()), coalescence_times, label=fr'$\beta$={beta:.2f}' if beta!=beta_true else r'$\beta=\tanh^{-1}(1/d)$', color=color)
...
sm.set_array([])
ticks = [0, beta_true, max_beta]
tick_labels = ['0', r'$\tanh^{-1}(1/d)$', f'{max_beta:.2f}']
...
fig.savefig(save_path + f'temp/ER_coal_time_vs_N_d_{d}.png')
fig.savefig(save_path + f'temp/ER_coal_time_vs_N_d_{d}.svg')
...
plt.vlines(beta_true, 0, max(log_T_coal_over_N)*10, color='k', linestyle='--', label=r'$\tanh^{-1}(1/d)$')
plt.xlabel(r'Inverse Temperature $\beta$')
plt.ylabel(r'$\log(T_{coal})/N$')
plt.legend()
plt.savefig(save_path + f'temp/ER_log_T_coal_over_N_vs_beta_d_{d}.png')
plt.savefig(save_path + f'temp/ER_log_T_coal_over_N_vs_beta_d_{d}.svg')
