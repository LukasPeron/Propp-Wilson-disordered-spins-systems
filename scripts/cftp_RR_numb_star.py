"""
This script runs the bounding chain algorithm forward in time to investigate the convergence properties of the bounding chain itself. We apply this to the Ising model and spin glass on a RR graph and observe how the number of "star" states (undecided spins) evolves over time for different inverse temperatures (betas). This can provide insights into the mixing time and convergence behavior of the CFTP algorithm.
...
beta_no_zero = None
# Define the normalizer early so we can use it to pick line colors based on actual beta values
...
# Define standard ticks based on the RR graph parameters
ticks = [0, beta_BD, max_beta]
tick_labels = [r'$\beta=0$', r'$\tanh^{-1}\left(\frac{1}{d}\right)$', f'{max_beta:.2f}']
...
plt.savefig(save_path+f'algo/RR_glassy_number_star_state_d_{d}_N_{N}.png')
plt.savefig(save_path+f'algo/RR_glassy_number_star_state_d_{d}_N_{N}.svg')