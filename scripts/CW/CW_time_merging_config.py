...
t_coal_beta = []
for beta in betas:
    print(f"Using beta={beta:.4f} (beta_pm={beta_pm:.4f})")
    t_coal= []
    for _ in range(10):
...
np.save(save_path + f"data/algo/complet/CW_{N}_configs_coal_time.npy", t_coal_beta)
...
plt.yscale("log")
plt.legend()
plt.savefig(save_path + f"figures/algo/complet/CW_{N}_configs_coal_time.png", dpi=300)
plt.close()