...
betas_upper = np.linspace(beta_pm, max_beta, 10)
...
for j, beta in enumerate(betas):
    ...
    # Plotting lines without labels since we will use a colorbar instead of a legend
...
# Create a Normalizer and a ScalarMappable for the colorbar
sm = mpl.cm.ScalarMappable(norm=norm, cmap=cmc.berlin)
sm.set_array([])

# Define standard ticks
ticks = [0, beta_pm, max_beta]
tick_labels = [r'$\beta=0$', r'$\beta_c$', f'{max_beta:.2f}']
...
plt.savefig(save_path+'algo/CW_number_star_state.png')
plt.savefig(save_path+'algo/CW_number_star_state.svg')